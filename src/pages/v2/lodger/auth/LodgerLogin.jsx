// src/pages/v2/lodger/auth/LodgerLogin.jsx
// Login Inquilinos — split-screen: foto izquierda + formulario derecho
...
const STORAGE_KEY = "login-welcome-lodger.jpg";
const BUCKET = "Assets-SmartRent";
const FALLBACK_GRADIENT = "linear-gradient(135deg, #2563eb, #7c3aed)";
...
export default function LodgerLogin() {
  const auth = useAuth();
  ...
  const resolvePostLogin = (authState) => {
    ...
    console.log("[LodgerLogin] resolvePostLogin:", { role, hasTenant, tenantState: authState.tenantState });
  ...
  useEffect(() => {
    ...
    if (!auth.loading && auth.user && auth.profile) return;

    timerRef.current = setTimeout(() => {
      console.warn("[LodgerLogin] profile load timed out after 8s");
  ...
  const onSubmit = (e) => {
    ...
    form.handleLogin(() => {
      console.log("[LodgerLogin] signIn succeeded, waiting for profile...");
    });
  };
  ...
  const renderFormPanel = () => {
    ...
    if (postLogin === "no_tenant") {
      return (
        <div style={s.formInner}>
          <h1 style={s.title}>Bienvenido a tu espacio</h1>
          <div style={s.msgBox}>
            <p style={s.msgText}>
              Tu cuenta no esta asociada a un alojamiento. Contacta con tu gestor para que te asigne un espacio.
            </p>
            <Link to="/v2" style={s.secondaryBtn}>
              Volver al inicio
            </Link>
          </div>
        </div>
      ...
      <div style={s.page}>
        ...
        <div className="sr-login-image-panel" style={{ ...s.imagePanel, ...imageStyle }}>
          {imageUrl && !imageFailed && (
            <img src={imageUrl} alt="SmartRoom Lodger" style={s.image} />
          )}
        </div>
