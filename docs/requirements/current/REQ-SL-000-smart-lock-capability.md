# REQ-SL-000: SmartLock — Capacidad de Acceso Inteligente
...
**Renombrado desde:** REQ-014-smart-access-lock.md (histórico, ver `docs/requirements/current/REQ-SL-000-smart-lock-capability.md` como única fuente de verdad vigente)

---

> **Nota de namespace:** este requirement es la fuente funcional del add-on **SmartLock** (`smart-lock`). Las decisiones normativas, contratos, guías de implementación, tests y diagramas derivados de este requirement viven en `docs/smart-lock/` siguiendo el estándar de `docs/_commons/rules/rules-01-document-authoring-standard.md`. El código de referencia usa el prefijo `sal-*` en Edge Functions (histórico, de "SmartAccessLock") y el prefijo `lock_*` en tablas de base de datos.
